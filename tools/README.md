...
After step 3, point `examples/model` at the work dir and run `npm run verify:headless` (it
serves the repo itself): the engine must reproduce the reference forward (cosine) and, for the
committed Bonsai fixtures, the exact known-good token ids.
