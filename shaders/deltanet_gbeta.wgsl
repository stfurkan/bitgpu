...
//   g[s,h]    = -exp(A_log[h]) * softplus(a[s,h] + dt_bias[h])     (<= 0, log-space decay)
//   beta[s,h] = sigmoid(b[s,h])
// per value head h. One invocation per (s,h). Output is [g (S*H) ; beta (S*H)] concatenated, so
// the engine binds two sub-ranges. Matches tools/qwen35_numpy._linear_attention.
...
@group(0) @binding(3) var<storage, read> a_log: array<f32>;    // [H]
