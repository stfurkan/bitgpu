...
struct Params { S: u32, H: u32, DK: u32, DV: u32 };
@group(0) @binding(0) var<uniform> p: Params;
@group(0) @binding(1) var<storage, read> q: array<f32>;     // [S, H, DK]
@group(0) @binding(2) var<storage, read> k: array<f32>;     // [S, H, DK]
...
fn main(@builtin(workgroup_id) wg: vec3<u32>, @builtin(local_invocation_id) lid: vec3<u32>) {
  ...
  for (var t = 0u; t < p.S; t = t + 1u) {
    let base = t * p.H + h;
    for (var i = lid.x; i < DK; i = i + WGV) { ksh[i] = k[base * DK + i]; qsh[i] = q[base * DK + i]; }
    workgroupBarrier();
    var sk = 0.0;
    var sq = 0.0;
